...
async def inline_handler(inline_query: types.InlineQuery):
    ...
    query_params = " ".join(query_text.split()[2:])
    query_param_text = " ".join(query_text.split(" ")[:2]) + " "

    try:
        items = await __get_inline_array(dicts.query_variants[query_param_text], query_params)
...
async def __get_inline_array(value, query_params: str):
    if value == "clan_users":
        try:
            array_items = await DB.select_clan_users(query_params)
        except ValueError:
            return
    elif value == "clans":
        array_items = await DB.select_all_clans(query_params)
    else:
        array_items = await DB.select_all_items(value, query_params, dicts.names_inline_array_types[value])

    if value == "opponents":
        return [await __get_input_text_content(i[1], f"🔸{i[3]}\n🗺{dicts.transitions_v2[i[7]]}", f"/getmob {i[0]}") for i in array_items]
    elif value == "items":
        return [await __get_input_text_content(i[1], f"{strings.rarity} {i[4]}", f"/getequip {i[0]}") for i in array_items]
...
async def __get_input_text_content(title: str, description: str, input_message_content: str) -> InlineQueryResultArticle:
    ...
    try:
        ...
        if len(command_info) == 2:
            ...
            if command == "getmob":
                ...
                if await have_access(message.from_user.id) and user_info.id != user_info_main_player.id:
                ...
                text = await strings.get_find_user_desc(user_info.id, user_info.nickname, user_info.lvl, user_info.glory, user_info.course, await user_info.get_glory_rank())
            else:
                text = strings.mob_desc_main_word[4]
                clan_info = Clan(*await DB.get_clan_info_with_clan_id(arg))
                text += f"{await clan_info.get_clan_emoji} {clan_info.name}\n\n{await clan_info.get_clan_leader_text}{await clan_info.get_clan_users_count()}{await clan_info.get_clan_power}"
                user_info = await get_user_info(message.from_user.id)
